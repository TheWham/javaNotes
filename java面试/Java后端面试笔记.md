...
1. 优先选择从选择配置中填的优先级最高的从节点, 比如服务器内存越大配的优先级越高
	![](assets/Java后端面试笔记/file-20260524002243203.png)

2. 其次会选择复制偏移量最大的节点, 毕竟它的数据是最接近原主节点的.
	![661](assets/Java后端面试笔记/file-20260524002320927.png)
3. 如果以上两个条件都相同就选择运行id最小的从节点 
	![](assets/Java后端面试笔记/file-20260524002334084.png)
...
**单个哨兵模式带来的问题:**
![](assets/Java后端面试笔记/file-20260524000425384.png)
1. 如果只有一个哨兵, 哨兵挂了 那就没人协调管理主从切换了依然存在单点问题
![](assets/Java后端面试笔记/file-20260524000444889.png)
2. 如果哨兵和redis之间存在网络问题哨兵还可能会误判

所以我们可以多加几个哨兵组成哨兵集群 哨兵之间相互通信; 
![](assets/Java后端面试笔记/file-20260524000500831.png)

单个哨兵认为redis节点下线了那就是主观下线此时它会跟多个哨兵进行确认只有在大多数哨兵都认为这个redis下线了才是真的下线了称为客观下线; 这样一个使用了哨兵集群和主从模式的redis架构就是哨兵模式可以极大的提升redis的可用性

![](assets/Java后端面试笔记/file-20260524000412386.png)


## 9. Redis集群模式
哨兵模式存在的问题, 主节点和从节点存的都是全量数据 但是单机服务器内存总有上限, 加再多的从节点 也无法突破单机瓶颈 如果想要存储更多的数据jiu'yao
![](assets/Java后端面试笔记/file-20260524002639246.png)
# 九、消息队列 MQ
