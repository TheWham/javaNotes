...
我理解的 Java 特性可以分几个层面来看。  
* 首先，Java 是一门面向对象语言，支持**封装、继承、多态**和**抽象**，这让代码更容易复用和扩展。  
* 其次，Java 有跨平台特性，也就是常说的“一次编写，到处运行”，Java 源码会先编译成字节码，然后由不同平台上的 JVM 执行。  
* 第三，Java 有自动内存管理机制，程序员不需要手动释放内存，JVM 会通过垃圾回收器回收不再使用的对象。  
* 第四，Java 提供了比较完善的异常处理机制，可以通过 checked exception 和 unchecked exception 来区分不同类型的异常。  
* 第五，Java 天生支持多线程，并且提供了 synchronized、volatile、Lock、线程池、并发集合、CompletableFuture 等并发工具。  
* 另外，Java 还有泛型、反射、注解、Lambda 表达式、Stream API、模块化等特性，这些在日常开发和框架中用得非常多。  
* 比如 Spring 里面大量使用了反射和注解，集合框架里大量使用泛型，业务代码里经常用 Stream 和 Lambda 简化数据处理。
### 1. 面向对象三大特性
...
1. **意义**
	多态其实是一种抽象行为. 它主要作用是让程序员可以面向对象编程而不是具体的实现类.这样写出来的代码拓展性会更强.
	
2. **运行时多态和编译时多态**
	编译时多态和运行时多态是面向对象编程中多态性的两种实现方式, 他们分别再不同的阶段决定方法的绑定

| 类型    |  别名  |            实现方式 | 绑定时机 |
| :---- | :--: | --------------: | ---- |
| 编译时多态 | 静态多态 | 方法重载Overloading | 编译期  |
| 运行时多态 | 动态多态 |  方法重写Overriding | 运行期  |

3. **编译时多态**
	编译时多态也叫静态多态, 是在编译阶段确定方法的调用, 主要通过方法重载实现.
	方法重载是指在同一个类中定义多个方法, 这些方法的名称相同但是参数列表不同, 包括参数的类型或数量不同. java编译器在编译时会根据方法调用时传入的参数类型和数量, 决定调用哪一个重载方法.
	
4. **运行时多态**
	运行时多态也叫动态多态, 是在运行是确定方法的调用, 通过方法重写实现.
	方法重写是子类重写父类的一个或多个方法.通过父类引用调用方法时, 实际执行的是子类重写 后的方法. 这种多态性是在运行时根据对象的实际类型决定的
	
5.  **虚拟方法表的底层原理**
	JVM在类加载的时候会给每个类创建一张虚拟方法表`vtable` , 里面存放的是方法的入口地址, 当子类重写了父类方法, 子类的`vtable` 中的对应的位置就会指向子类的自己实现的方法,而不是父类的.
	当通过父类引用调用方法时, **JVM**会根据**对象头里存的实际类型信息**, 找到对应的`vtable`,然后 根据**方法签名**在`vtable` 里查找**方法入口地址**, 最总跳转执行, 这个过程叫做**动态分派**, 发生在运行时候, 所以叫做**运行时多态**.
...
---
### 提问
![590](assets/Java后端面试笔记/file-20260527185656355.png)
...
既然单个 Redis 节点怕宕机, 就可以给它加副本, 然后把这些节点分成主从关系。

主节点负责对外提供写操作, 写入后再把数据同步给从节点。从节点也不是闲着, 它可以对外提供读操作, 从节点越多, 整体能承受的读请求量就越高。

如果主节点宕机, 可以把某个从节点切换成新的主节点。主从 Redis 一般会部署在不同服务器上, 这样既提升了 Redis 的可用性, 也提升了读性能。

### 主从复制细节

**问题来了, 主节点跑了很久里面有很多缓存, 现在突然新增一个从节点, 主节点是如何同步数据到从节点呢?**

Redis 主从复制可以分为全量复制和部分复制。

- 第一次同步时, 主节点会生成 RDB 快照, 把 RDB 文件发送给从节点。
- 从节点加载 RDB 后, 就能拿到主节点当时的大部分数据。
- 之后主节点再把同步期间产生的新写命令继续发给从节点。

**如果同步过程中还有一些写操作呢?**

同步过程中的写操作会记录到主节点的复制积压缓冲区中。等 RDB 同步完成之后, 主节点会继续把缓冲区里的命令发给从节点。

复制积压缓冲区可以理解成一个固定大小的内存环形数组, 里面保存最近一段时间的写命令。主从之间会通过复制偏移量 `offset` 记录同步进度。如果从节点只是短暂断线, 恢复连接后就可以根据偏移量做部分重同步, 不一定每次都全量复制。

![553](assets/Java后端面试笔记/file-20260523224807044.png)

![551](assets/Java后端面试笔记/file-20260523224726786.png)

**主从切换的问题:**

经典主从模式下, 主节点挂了之后需要手动切换主从节点。从故障发生到人工介入会有时间差, 所以有些场景会借助 `keepalived` 这类自动脚本工具。

但是这类脚本工具只能粗略判断 Redis 是否存活, 比如端口是否响应、进程是否存在。它感知不到 Redis 内部的复制偏移量、数据同步进度、从节点健康状态等信息。所以在多从节点场景下, `keepalived` 选出来的从节点不一定是最适合接管主节点的那个。

## 7. 哨兵模式

> 我们可以在原来主从模式的基础上加入一个监控 Redis 内部状态的进程。监控进程先从主节点获取当前有哪些从节点, 然后定时 `ping` 主从节点。一旦发现主节点挂了, 就从健康的从节点里选一个顶上去。
...
1. 优先选择从节点配置中优先级最高的从节点, 比如服务器内存越大, 配置的优先级越高

   ![545](assets/Java后端面试笔记/file-20260524002243203.png)

2. 其次会选择复制偏移量最大的节点, 毕竟它的数据是最接近原主节点的.

   ![539](assets/Java后端面试笔记/file-20260524002320927.png)

3. 如果以上两个条件都相同, 就选择运行 ID 最小的从节点

   ![531](assets/Java后端面试笔记/file-20260524002334084.png)

这个监控进程就是所谓的哨兵 Sentinel。它的核心功能是监控、故障判断、主从切换和通知客户端。

Sentinel 需要跟 Redis 通信, 所以会复用 Redis 底层的一些通信协议能力。Redis 和 Sentinel 本质上是同一套代码通过不同启动参数部署出来的两个服务进程。

## 8. 哨兵集群

### 单个哨兵模式带来的问题

![568](assets/Java后端面试笔记/file-20260524000425384.png)

1. 如果只有一个哨兵, 哨兵挂了, 那就没人协调管理主从切换了, 依然存在单点问题

![561](assets/Java后端面试笔记/file-20260524000444889.png)

2. 如果哨兵和 Redis 之间存在网络问题, 哨兵还可能会误判

所以我们可以多加几个哨兵组成哨兵集群, 哨兵之间相互通信。

![550](assets/Java后端面试笔记/file-20260524000500831.png)

单个哨兵认为 Redis 节点下线, 这叫主观下线。此时它会跟其他哨兵确认, 只有达到配置的 `quorum` 数量后, 才会认为主节点客观下线。

确认客观下线后, 哨兵之间还会选举出一个 leader, 由 leader 负责执行故障转移, 选择一个合适的从节点升级为新的主节点。

这种使用哨兵集群 + 主从模式的 Redis 架构, 就是哨兵模式。它能解决主从模式下人工切换慢、单点判断不可靠的问题, 可以明显提升 Redis 的可用性。

![529](assets/Java后端面试笔记/file-20260524000412386.png)

## 9. Redis Cluster 核心原理

哨兵模式能提升可用性, 但主节点和从节点存的还是全量数据。单机服务器内存和 QPS 总有上限, 加再多从节点也无法突破单机存储瓶颈。

如果想存储更多数据, 就要引入 Redis Cluster, 通过分片把数据拆到多个 Redis 节点上。

![516](assets/Java后端面试笔记/file-20260524002639246.png)

![523](assets/Java后端面试笔记/file-20260524002838160.png)

### 数据切分

单机内存有限, 数据量不断增长, 用一个 Redis 节点承载所有数据不合理。

所以可以把数据切成多份, 分别放到多个 Redis 节点上。数据量变大后, 再根据需要增加 Redis 节点, 这样就具备了横向扩展能力。

### 早期想法: 直接按节点数取模

可以对多个 Redis 节点进行编号, 然后用 key 的 hash 值跟 Redis 节点总数取模。

```text
redis_num = CRC16(key) mod redis_all_num
```

![632](assets/Java后端面试笔记/file-20260524003336094.png)

但是这种方式有明显问题。如果前期没有规划好 `redis_all_num` 的容量, 后面节点数量一变, `redis_num` 的计算结果也会跟着变, 最后大概率导致大量数据都要重新迁移, 成本和风险都很高。

### 解决方案: 引入哈希槽

Redis Cluster 在 key 和 Redis 节点之间加了一层哈希槽。哈希槽数量固定为 16384, 每个 key 先计算出自己属于哪个 slot。

```text
slot_num = CRC16(key) mod 16384
```

然后每个 Redis 节点负责一部分槽位。

![620](assets/Java后端面试笔记/file-20260524005424383.png)

与此同时, 每个 Redis 节点都会维护其他 Redis 节点负责哪些槽位的信息。有了这些信息, 任意一个 Redis 节点只要拿到 key, 就能知道它应该由哪个 Redis 节点处理。

![631](assets/Java后端面试笔记/file-20260524005935652.png)

如果现在想增加 Redis 节点, 因为哈希槽这一层固定存在, 16384 个 slot 不变, key 计算出来的 `slot_num` 也不会变。我们只需要把一部分 slot 分配给新增节点, 然后迁移这些 slot 对应的数据即可, 不需要全量重新迁移。

![529](assets/Java后端面试笔记/file-20260524010239956.png)

### 重定向

引入哈希槽后, 客户端需要知道 key 对应哪个 slot, 以及这个 slot 由哪个 Redis 节点负责。

Redis 提供了 `CLUSTER SLOTS` 命令, 客户端可以通过这个命令获取最新的哈希槽分配信息, 并缓存到本地。

![525](assets/Java后端面试笔记/file-20260524011503835.png)

如果 Redis 节点数量有变化, 客户端没有及时更新本地 slot 信息, 也不会直接出大问题。接收到请求的 Redis 节点会根据自己维护的 slot 信息判断这个 key 是否属于自己。

- 如果属于自己, 就直接完成读写。
- 如果不属于自己, 就返回重定向信息, 告诉客户端应该去哪个节点访问。

![598](assets/Java后端面试笔记/file-20260524011834089.png)

重定向里有两个关键命令:

- `MOVED`: slot 已经稳定迁移到另一个节点, 客户端应该更新本地 slot 缓存。
- `ASK`: slot 正在迁移中, 客户端临时去目标节点访问, 一般不立刻整体更新 slot 缓存。

### 数据迁移中的访问问题

如果调整 Redis 节点个数后, 数据正在迁移, 客户端访问某个节点时发现没有数据, 需要区分是真的没有这个 key, 还是 key 正在迁移中。

Redis 内部会维护 slot 是否正在迁移的状态。如果正在迁移, Redis 节点会返回重定向指令, 提示客户端去另一个节点读取这个数据。客户端根据重定向提示访问目标节点, 如果目标节点也没有, 才说明这个 key 大概率真的不存在。

![533](assets/Java后端面试笔记/file-20260524012015906.png)

![539](assets/Java后端面试笔记/file-20260524012428014.png)

## 10. Redis Cluster 读写和高可用

![640](assets/Java后端面试笔记/file-20260524115054992.png)

如果 Cluster 里某个 Redis 节点挂掉, 它负责的那部分 slot 就可能无法访问。所以生产环境下不会只部署多个 master 节点, 还会给每个 master 节点配置 replica。

这种把数据分片到多个 Redis master 节点, 同时每个 master 又有 replica 兜底的架构, 就是 Redis Cluster 常见的高可用部署方式。

![632](assets/Java后端面试笔记/file-20260524115326399.png)

### 读写 Redis Cluster

![636](assets/Java后端面试笔记/file-20260524162027201.png)

**写操作:**

假设客户端要写入的 `key` 是 `user:1001`, `value` 是 `xcs`。

成熟客户端一般会缓存 slot 信息, 正常情况下会先计算 key 对应的 slot, 然后直接访问负责这个 slot 的 Redis 节点。

如果客户端没有 slot 信息, 或者 slot 缓存过期, 它可能会先访问某个 Redis 节点。比如访问节点 A 后, A 会计算这个 key 对应的哈希槽, 假设结果是 1234。如果 1234 在 A 节点负责的范围内, 就直接写入 A 节点。

![519](assets/Java后端面试笔记/file-20260524162705824.png)

如果这个 key 计算出来的 slot 是 5462, 而 5462 属于节点 B, 那么节点 A 会返回 `MOVED` 重定向, 告诉客户端应该访问节点 B。客户端再重定向到节点 B 完成写入。

![525](assets/Java后端面试笔记/file-20260524162657686.png)

**读操作:**

假设客户端要读取的依然是 `user:1001`。

客户端会根据本地 slot 缓存找到负责该 key 的节点, 然后发送 `GET user:1001` 操作。如果缓存不准确, Redis 节点也会通过重定向告诉客户端应该去哪个节点读取。

![578](assets/Java后端面试笔记/file-20260524163000982.png)

如果节点 A 计算后发现 `user:1001` 对应的 slot 是 1234, 并且 1234 归自己管, 就直接返回对应的值 `xcs`。

如果计算出来的 slot 是 5462, 属于节点 B, 那么节点 A 会返回重定向指令, 告诉客户端这个 key 在节点 B, 客户端再访问节点 B 完成读取。

![574](assets/Java后端面试笔记/file-20260524163244496.png)

### 高可用切换

Redis Cluster 中每个 master 一般都会挂至少一个 replica。

- master 正常时, 默认由 master 负责读写。
- master 宕机后, Cluster 会从它的 replica 中选出新的 master。
- 故障恢复后, 原来的 master 可以作为 replica 重新加入集群。

注意, Redis Cluster 默认读写都走 master。读 replica 不是默认行为, 需要客户端支持并开启只读访问能力, 比如发送 `READONLY`。

### 多 key 操作限制

Redis Cluster 中, 多 key 命令要求这些 key 落在同一个 slot。如果多个 key 分布在不同 slot, 可能会出现 `CROSSSLOT` 错误。

如果业务确实需要让多个 key 落在同一个 slot, 可以使用 hash tag:
...
Redis 只会对 `{1001}` 这部分计算 hash, 所以上面两个 key 会落到同一个 slot。

### 面试话术

> Redis Cluster 主要解决单机 Redis 内存和 QPS 上限问题。它不是简单按节点数取模, 而是引入了 16384 个 hash slot。每个 key 先通过 CRC16 计算出 slot, 再由负责这个 slot 的节点处理。客户端通常会缓存 slot 映射, 如果映射过期, Redis 会通过 `MOVED` 或 `ASK` 做重定向。生产环境下每个 master 还会配 replica, master 挂了以后 replica 可以被提升为新的 master。需要注意的是, Cluster 下多 key 操作有跨 slot 限制, 必要时可以用 hash tag 控制 key 落到同一个 slot。
