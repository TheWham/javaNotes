...
### 1. 基本原理:
既然单个节点怕崩, 就是加副本; 然后将它们分为主从关系, 主节点向外提供读写操作, 然后同步给从节点, 从节点也不能闲着一样可以向外提供读操作, 加到从节点越多支持读请求量就越高, 主节点崩了就可以手动切换, 将从节点作为新的主节点. 将主从redis分别部署在不同的服务器上 , 这样及提升了redis的可用性也增加了读性能
### 2. 主从复制的细节

**问题来了, 主节点跑了很久里面有很多缓存, 现在突然新增一个从节点, 主节点是如何同步数据到从节点呢?**
* redis为了尽可能保证数据持久性,会定期将全量内存数据持久化到磁盘文件中也就是RDB文件, 主节点可以将RDB文件传给从节点完成大部分的数据同步

**如果同步过程中还有一些写操作呢?**
* 可以将它记录到主节点中的复制积压缓冲区中. 等RDB同步结束之后, 再将里面的内容同步到从节点. 说白了它就是个固定大小的内存数组 下标就是它的复制偏移量, 可以用偏移量来跟踪主从复制之间的进度; 这样就算主节点在工作过程中突然挂掉. 重启之后也可以根据偏移量来完成剩余的复制
* ![553](assets/Java后端面试笔记/file-20260523224807044.png)
* ![551](assets/Java后端面试笔记/file-20260523224726786.png)

**主从切换的问题:**

* 经典主从模式, 主节点挂了之后是需要手动切换主从节点的, 从发生问题到人工介入需要一定的时间差, 所以需要借助一些像keepalived这样的自动脚本工具, 但是这些脚本工具只能粗糙的检测redis是否存活.它可以感知端口是否相应, 进程是否存在, 但是它不能感知内部的复制偏移量和数据一致性等状态, 所以在多从节点的场景下, keepalive选中的从节点不一定是最合适的那一个.

## 7. 哨兵模式

>我们可以在原来主从模式的基础上加入一个监控 redis内部状态的进程. 监控进程先跟主节点获取当前有哪些从节点, 在每秒ping一下从节点, 一旦发现主节点挂了 就立刻选一个健康的从节点顶上去

**keepalived缺陷解决方案:**
* 引入哨兵;

**选择从节点的方式:**
1. 优先选择从选择配置中填的优先级最高的从节点, 比如服务器内存越大配的优先级越高
...
3. 如果以上两个条件都相同就选择运行id最小的从节点 
	![531](assets/Java后端面试笔记/file-20260524002334084.png)


这个监控进程就是所谓的哨兵sentinel 它的功能不多但是还需要跟redis进行通信也就是需要复用redis底层的一些通信协议能力, 所以可以直接将它放到redis的代码仓库里, 也就是说redis和sentinel本质上是同一套代码通过不同的启动参数部署出来的两个服务进程而已

## 8. 哨兵集群

**单个哨兵模式带来的问题:**
![568](assets/Java后端面试笔记/file-20260524000425384.png)
1. 如果只有一个哨兵, 哨兵挂了 那就没人协调管理主从切换了依然存在单点问题
![561](assets/Java后端面试笔记/file-20260524000444889.png)
2. 如果哨兵和redis之间存在网络问题哨兵还可能会误判

所以我们可以多加几个哨兵组成哨兵集群 哨兵之间相互通信; 
![550](assets/Java后端面试笔记/file-20260524000500831.png)

单个哨兵认为redis节点下线了那就是主观下线此时它会跟多个哨兵进行确认只有在大多数哨兵都认为这个redis下线了才是真的下线了称为客观下线; 这样一个使用了哨兵集群和主从模式的redis架构就是哨兵模式可以极大的提升redis的可用性
...
哨兵模式存在的问题, 主节点和从节点存的都是全量数据 但是单机服务器内存总有上限, 加再多的从节点 也无法突破单机瓶颈 如果想要存储更多的数据就要引入Redis集群模式
![516](assets/Java后端面试笔记/file-20260524002639246.png)
![523](assets/Java后端面试笔记/file-20260524002838160.png)

**数据切分:** 
 单机内存有限而数据无限 拿有限的单机内存去支持无限的数据明显不合理; 所以既然数据无限就将数据切成多份 将多份数据放到多个Redis节点上 数据量变大后 再根据需要增加redis节点 也就是做到了高可扩展
...
但是这种方式存在很大的问题, 如果前期没有规划好`redis_all_num`容量, 那么总数量就会改变 `redis_num`也会改变最后大概率导致 所有数据都要进行重新迁移 成本风险太高
**解决方案: 引入哈希槽**
要想让`redis_num` 不改变就要让`redis_all_num` 固定下来, 再key和redis节点之间加上哈希槽 固定长度数组, 长度=16384, `slot_num = CRC16(key) mod 16384`, 之后每个redis节点负责一部分的槽位

![620](assets/Java后端面试笔记/file-20260524005424383.png)
于此同时每个redis节点之间维护了其他redis管理了哪些槽这一信息, 有了这些信息 任意一个redis节点只要拿到key就能知道它该存到哪个redis节点上;
![631](assets/Java后端面试笔记/file-20260524005935652.png)

现在如果我们想要增加redis节点个数, 由于哈希槽这一层的存在, 16384固定不变, 那计算出的slot_num 也不会变化, 我们只需要定义新增的redis负责哪部分哈希槽, 然后迁移那一部分数据即可, 比以前要进行全部数据重新迁移 的工作量大大减小
![529](assets/Java后端面试笔记/file-20260524010239956.png)

**重定向:**
引入哈希槽这个概念之后, 就可以用多个redis节点来切分存储数据了, 但这也引出了另外一个问题, 哈希槽是维护在redis里面的, 客户端没有这个信息, 读写数据时候怎么知道这些key对应哪个哈希槽和redis节点, 解法是 redis提供一个cluster slots命令可以让客户端获取到最新的哈希槽分配信息;
![525](assets/Java后端面试笔记/file-20260524011503835.png)

如果redis节点数量有变化, 客户端没有及时更新哈希槽信息是不是就会出错? 完全不用担心, 客户端
只管对任意一个redis节点进行读写就行, 接收到数据的redis节点会根据最新维护的哈希槽信息判断这个key属于哪个redis节点, 如果恰好属于它自己, 则直接完成读写, 否则redis节点就返回一个错误, 并告诉客户端该去哪个redis节点读写数据就好, 这个过程也叫重定向

![598](assets/Java后端面试笔记/file-20260524011834089.png)

**结合重定向的数据迁移问题:**

如果调整了redis个数之后, 数据正在进行迁移行为,客户端查询某个redis节点没数据, 我怎么知道它是真没有这个key还是没有迁移过来呢?
* redis内部会对redis的哈希槽维护一个是否正在迁移数据的状态, 如果正在迁移中, redis节点会返回一个重新向指令告诉客户端接下来该去哪个节点读取这个数据, 客户端根据重定向的指令的提示重新访问另一个redis节点, 如果还是没数据就表示真没有数据
...
如果其中有个redis节点挂掉就会导致部分数据无法访问, 所以可以为每个redis节点加几个从节点; 这样将数据分片到多个Redis节点上, 同时为每个节点加入从节点的架构, 就是所谓的集群模式
...
**写操作:**
假设客户端要写入的`key`是 `user:1001` `value`是 `xcs` 这时候客户端会随机访问一个Redis节点(如果客户端有slot信息, 则不是随机连), 比如节点A, 客户端像节点A发送`set key value` 的操作 redis会计算这个key 对应的哈希槽的值比如说1234, A节点发现在自己0-5460数据的范围内, 所以就会被写入到节点A中.
![519](assets/Java后端面试笔记/file-20260524162705824.png)

  如果`key`值是5462 属于B节点管那就会返回一个重定向命令, 告诉客户端这个应该写到B, 客户端会自动重定向到节点B 完成写入
![525](assets/Java后端面试笔记/file-20260524162657686.png)


**读操作:**
假设客户端尧都区的依然是`user:1001`客户端还是会随机连接到某一个节点比如节点A(如果客户端有slot信息, 则不是随机连) 发送`get key` 操作
![578](assets/Java后端面试笔记/file-20260524163000982.png)

节点A计算`key`的值发现`user:1001` 其对应的哈希值是1234归自己管那么就会返回对应的值 `xcs`, 
如果计算的哈希槽的值是5462属于节点B,那么节点A会返回一个重定向指令告诉客户端这个`key`在节点B, 客户端会自动重定向到节点B完成数据的读取
