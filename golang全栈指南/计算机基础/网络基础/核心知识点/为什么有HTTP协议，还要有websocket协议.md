...
从 [HTTP 协议](硬核！漫画图解HTTP知识点_面试题.md)的角度来看，就是点一下网页上的某个按钮，**前端发一次 HTTP 请求，网站返回一次 HTTP 响应**。
...
![创建角色页面](https://cdn.xiaobaidebug.top/创建角色页面.jpeg)
...
![使用HTTP定时轮询](https://cdn.xiaobaidebug.top/使用HTTP定时轮询3.drawio.png)
...
![长轮询](https://cdn.xiaobaidebug.top/长轮询.drawio.png)
...
![RocketMQ的消费者通过长轮询获取数据](https://cdn.xiaobaidebug.top/RocketMQ的消费者通过长轮询获取数据.drawio.png)
...
![websocket在四层网络协议中的位置](https://cdn.xiaobaidebug.top/websocket在四层网络协议中的位置.drawio.png)
...
![base64转为新的字符串](https://cdn.xiaobaidebug.top/base64转为新的字符串.drawio.png)

之后，浏览器也用同样的**公开算法**将`base64码`转成另一段字符串，如果这段字符串跟服务器传回来的**字符串一致**，那验证通过。

![对比客户端和服务端生成的字符串](https://cdn.xiaobaidebug.top/对比客户端和服务端生成的字符串.drawio.png)

就这样经历了一来一回两次 HTTP 握手，websocket 就建立完成了，后续双方就可以使用 webscoket 的数据格式进行通信了。

![建立websocket连接.drawio](https://cdn.xiaobaidebug.top/建立websocket连接.drawio.png)
...
![服务器同意升级为websocket协议](https://cdn.xiaobaidebug.top/未命名绘图ddddd.drawio.png)
...
![HTTP和websocket的关系](https://cdn.xiaobaidebug.top/HTTP和websocket的关系.drawio.png)
...
![websocket报文格式](https://cdn.xiaobaidebug.top/websocket报文格式2.drawio.png)
...
![payload长度在0到125之间](https://cdn.xiaobaidebug.top/payload长度在0到125之间2.png)

- 如果是`126（0x7E）`。那它表示 payload 的长度范围在 `126~65535` 之间，接下来还需要**再读 16bit**。这 16bit 会包含 payload 的真实长度。

![payload长度在126到65535之间](https://cdn.xiaobaidebug.top/payload长度在126到65535之间2.png)

- 如果是`127（0x7F）`。那它表示 payload 的长度范围`>=65536`，接下来还需要**再读 64bit**。这 64bit 会包含 payload 的长度。这能放 2 的 64 次方 byte 的数据，换算一下好多个 TB，肯定够用了。

![payload长度大于等于65536的情况](https://cdn.xiaobaidebug.top/payload长度大于等于65536的情况.png)
...
![消息边界长度标志](https://cdn.xiaobaidebug.top/消息边界长度标志6.drawio.png)
...
![websocket的使用场景](https://cdn.xiaobaidebug.top/websocket的使用场景.drawio.png)
...
![](https://cdn.xiaobaidebug.top/image-20220522162506224.png)
**离开广东好长时间了，好久没人叫我靓仔了。**
...
![](https://cdn.xiaobaidebug.top/image-20220522162616202.png)![](https://cdn.xiaobaidebug.top/006APoFYly1g5q9gn2jipg308w08wqdi.gif)
