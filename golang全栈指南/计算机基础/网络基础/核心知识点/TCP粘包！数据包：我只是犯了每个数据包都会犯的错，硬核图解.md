![默认文件1616129034230](https://cdn.xiaobaidebug.top/image/默认文件1616129034230.png)
...
![四层网络协议](https://cdn.xiaobaidebug.top/image/四层网络协议1.png)
...
![数据分片](https://cdn.xiaobaidebug.top/image/数据分片1.png)
...
![正常发送数据包](https://cdn.xiaobaidebug.top/image/正常发送数据包2.png)
...
- 加入特殊标志
  ...
  ![消息边界头尾标志](https://cdn.xiaobaidebug.top/image/消息边界头尾标志.png)
...
这个一般配合上面的特殊标志一起使用，在收到头标志时，里面还可以带上消息长度，以此表明在这之后多少 byte 都是属于这个消息的。如果在这之后正好有符合长度的 byte，则取走，作为一个完整消息给应用层使用。在实际场景中，HTTP 中的`Content-Length`就起了类似的作用，当接收端收到的消息长度小于 Content-Length 时，说明还有些消息没收到。那接收端会一直等，直到拿够了消息或超时，关于这一点[上一篇文章](硬核！漫画图解HTTP知识点_面试题.md)里有更详细的说明。
...
![消息边界头尾加校验标志](https://cdn.xiaobaidebug.top/image/消息边界头尾加校验标志.png)
...
![ip报头](https://cdn.xiaobaidebug.top/image/ip报头2.png)

注意这里面是有一个 16 位的总长度的，意味着 IP 报头里记录了整个 IP 包的总长度。接着我们再看下 **UDP 的报头**。

![UDP报头](https://cdn.xiaobaidebug.top/image/udp报头4.png)
...
![UDP数据长度](https://cdn.xiaobaidebug.top/image/UDP数据长度-20210319074710451.png)
...
![tcp报头2](https://cdn.xiaobaidebug.top/image/tcp报头2.png)
...
![为什么UDP要冗余一个长度字段](https://cdn.xiaobaidebug.top/image/为什么UDP要冗余一个长度字段.png)
...
![P分包与重组](https://cdn.xiaobaidebug.top/image/P分包与重组.png)
