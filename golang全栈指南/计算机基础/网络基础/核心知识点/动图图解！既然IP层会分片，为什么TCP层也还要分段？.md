![](https://cdn.xiaobaidebug.top/image/目录.png)
...
![数据分片](https://cdn.xiaobaidebug.top/image/数据分片2.png)
...
那**MSS 和 MTU 是什么关系**呢？这个在[之前的文章](TCP粘包！数据包：我只是犯了每个数据包都会犯的错，硬核图解.md)里简单提到过。这里单独拿出来。
...
![MSS分包](https://cdn.xiaobaidebug.top/image/MSS分包.gif)

### 如何查看 MSS？

我们都知道 TCP 三次握手，而`MSS`会在三次握手的过程中传递给对方，用于通知对端本地最大可以接收的 TCP 报文数据大小（不包含 TCP 和 IP 报文首部）。

![抓包mss](https://cdn.xiaobaidebug.top/image/抓包mss.png)
...
![TCP报头](https://cdn.xiaobaidebug.top/image/tcp报头5.png)
...
![MTU分包](https://cdn.xiaobaidebug.top/image/mtu分包.gif)
...
![假设TCP不分段](https://cdn.xiaobaidebug.top/image/TCP分片1.gif)
...
![假设TCP分段](https://cdn.xiaobaidebug.top/image/TCP分段.gif)
...
![IP分片再分片](https://cdn.xiaobaidebug.top/image/IP分片再分片.gif)
...
![IP报头DF](https://cdn.xiaobaidebug.top/image/ip报头DF1.png)
...
![获得pmtu](https://cdn.xiaobaidebug.top/image/获得pmtu.gif)
...
![获得pmtu后的TCP重传](https://cdn.xiaobaidebug.top/image/获得pmtu后的TCP重传.gif)
