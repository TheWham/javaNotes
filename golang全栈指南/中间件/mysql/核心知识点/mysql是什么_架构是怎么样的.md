...
![mysql是什么](https://cdn.xiaobaidebug.top/mysql是什么配图.0001.jpeg)
...
![mysql是数据和应用的中间层](https://cdn.xiaobaidebug.top/mysql是什么配图.002.jpeg)

## 什么是 mysql

Mysql数据库，是一款存放和管理数据的软件, 它介于**应用**和**数据**之间，通过一些设计，将大量数据，变成一张张像 excel 的数据表。为应用提供创建(Create), 读取(Read), 更新(Update), 删除(Delete)等核心操作。

![mysql是什么](https://cdn.xiaobaidebug.top/mysql是什么配图.003.jpeg)
...
![ibd文件是什么](https://cdn.xiaobaidebug.top/mysql是什么配图.004.jpeg)

数据表越大，磁盘上的 ibd 文件也就越大。

直接读写一个大文件里的全部数据会很慢，所以 MySQL 将数据**拆成一个个数据页**，每页大小 **16KB**。这样我们读写部分表数据的时候，就只需要读取磁盘里的**几个数据页**就好。

![mysql将文件分成多个数据页](https://cdn.xiaobaidebug.top/mysql是什么配图.005.jpeg)
...
![B+树索引](https://cdn.xiaobaidebug.top/mysql是什么配图.006.jpeg)

上面提到的是针对主键的索引，也就是**主键索引**。

![主键索引](https://cdn.xiaobaidebug.top/mysql是什么配图.007.jpeg)

按同样的思路，也可以为**其他数据表的列**去建立**索引**，比如用户表的名称字段，这样我们就能快速查找到名字为 xx 的用户有哪些，这就是所谓的**辅助索引**。

![辅助索引](https://cdn.xiaobaidebug.top/mysql是什么配图.008.jpeg)
...
![buffer pool是什么](https://cdn.xiaobaidebug.top/mysql是什么配图.009.jpeg)
...
![buffer pool直接IO](https://cdn.xiaobaidebug.top/mysql是什么配图.010.jpeg)
...
![哈希的key和value](https://cdn.xiaobaidebug.top/mysql是什么配图.012.jpeg)

这个 hash 表，就是所谓的**自适应哈希索引**，**Adaptive Hash Index**。

![自适应哈希](https://cdn.xiaobaidebug.top/mysql是什么配图.013.jpeg)
...
![更新主键和辅助索引](https://cdn.xiaobaidebug.top/mysql是什么配图.014.jpeg)
...
![Change Buffer的更新流程](https://cdn.xiaobaidebug.top/mysql是什么配图.015.jpeg)

## Undo Log

在数据库中，有一个叫**事务**的概念。不了解没关系，说白了，就是可以让多行数据，要么**同时**更新成功，要么**同时**更新失败。也就是所谓的**原子性**。

![事务是什么](https://cdn.xiaobaidebug.top/mysql是什么配图.016.jpeg)

为了实现这一点，我们就需要知道写数据时每行数据**原来长啥样**，方便对更新后的数据行，进行**回滚**，因此就有了 **Undo Log**。

![undo log回滚](https://cdn.xiaobaidebug.top/mysql是什么配图.017.jpeg)
...
![undo log的写入流程](https://cdn.xiaobaidebug.top/mysql是什么配图.018.jpeg)
...
![redo log的写入流程](https://cdn.xiaobaidebug.top/mysql是什么配图.019.jpeg)

这时候问题就来了，我有这功夫更新 redo log file 文件，直接将 buffer pool 的数据写入到磁盘不香吗？

![为什么不直接写磁盘数据页](https://cdn.xiaobaidebug.top/mysql是什么配图.020.jpeg)
...
![顺序写比随机写快很多](https://cdn.xiaobaidebug.top/mysql是什么配图.0211.jpeg)
...
![innodb提供的函数接口](https://cdn.xiaobaidebug.top/mysql是什么配图.023.jpeg)
...
![server层](https://cdn.xiaobaidebug.top/mysql是什么配图.024.jpeg)
...
![server层做了哪些事情](https://cdn.xiaobaidebug.top/mysql是什么配图.025.jpeg)

**server 层**和**存储引擎层**共同构成了一个完整的数据库，它就是我们常说的 **Mysql 数据库**。

![mysql是什么](https://cdn.xiaobaidebug.top/mysql是什么配图.026.jpeg)

并且，server 层和存储引擎层是通过接口函数进行**解耦**的，换句话说就是，只要实现了上面这些接口函数，就能作为存储引擎与 Server 层对接。

![引擎解耦](https://cdn.xiaobaidebug.top/mysql是什么配图.027.jpeg)

比如，mysql 早期用的是 **myisam** 存储引擎，后来才支持的 **innodb**。

![常用存储引擎有哪些](https://cdn.xiaobaidebug.top/mysql是什么配图.028.jpeg)
...
![binlog写入流程](https://cdn.xiaobaidebug.top/mysql是什么配图.0291.jpeg)
...
![mysql架构总览图](https://cdn.xiaobaidebug.top/mysql是什么配图.030.jpeg)
