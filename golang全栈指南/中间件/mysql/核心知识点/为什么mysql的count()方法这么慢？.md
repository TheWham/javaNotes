...
![sms建表sql](https://cdn.xiaobaidebug.top/sms建表sql.png)

![sms表](https://cdn.xiaobaidebug.top/sms表.drawio.png)

需要注意的是**state字段，为0的时候说明这时候短信还未发送。**

此时还会有一个**异步线程**不断的捞起**未发送（state=0）**的短信数据，执行发短信操作，发送成功之后state字段会被**置为1（已发送）**。也就是说**未发送的数据会不断变少**。

![异步线程发送短信](https://cdn.xiaobaidebug.top/异步线程发送短信.drawio.png)
...
![Mysql架构](https://cdn.xiaobaidebug.top/Mysql架构56.drawio-20220613163829373.png)
...
![四层隔离级别](https://cdn.xiaobaidebug.top/四层隔离级别.png)

innodb引擎通过MVCC实现了**可重复隔离级别**，事务开启后，多次执行同样的**select快照读**，要能读到同样的数据。

于是我们看个例子。

![为什么innodb不单独记录表行数](https://cdn.xiaobaidebug.top/为什么innodb不单独记录表行数.drawio.png)
...
![](https://cdn.xiaobaidebug.top/u=3123397807,3516361318&fm=253&fmt=auto&app=138&f=JPEG.jpeg)那有没有其他更好的办法？
...
![explain里的rows](https://cdn.xiaobaidebug.top/explain里的rows.png)
...
![count_table表保存各种场景下的count](https://cdn.xiaobaidebug.top/保存各种场景下的count.drawio.png)
...
![将更改表行数的操作放入到事务里](https://cdn.xiaobaidebug.top/将更改表行数的操作放入到事务里.drawio.png)
...
![batch分批获取短信表](https://cdn.xiaobaidebug.top/batch分批获取短信表.drawio.png)

当然如果有条件，这种场景最好的方式还是**消费binlog将数据导入到hive里**，然后在hive里做查询，不少公司也已经有现成的组件可以做这种事情，不用自己写脚本，岂不美哉。

![mysql同步hive](https://cdn.xiaobaidebug.top/mysql同步hive.drawio.png)
...
关注公众号:【小白debug】
![](https://cdn.xiaobaidebug.top/扫码_搜索联合传播样式-标准色版.png)
