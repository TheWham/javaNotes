...
可以参考[kafka是什么,应用场景有哪些](核心知识点/kafka是什么？架构是怎么样的？.md)
Kafka 是一个分布式流式处理平台。
...
- 极致的性能：基于 Scala 和 Java 语言开发，设计中大量使用了**批量处理和异步**的思想，最高可以每秒处理千万级别的消息。
...
可以参考[kafka是什么，架构原理是什么](核心知识点/kafka是什么？架构是怎么样的？.md)
...
可以参考[replicas的概念](核心知识点/kafka是什么？架构是怎么样的？.md)
...
**Kafka 通过“同 key 进同 partition + partition 内 offset 顺序追加 + 同组内 partition 单消费者消费”来保证同一个业务对象的消息有序。**
