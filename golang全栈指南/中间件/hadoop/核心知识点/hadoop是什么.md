...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.002.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.003.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.004.png)

对小份数据进行**存储**，**计算**等一系列操作。

所以 Hadoop 要解决的核心问题有两个，一个是**怎么存**，另一个是**怎么算**？

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.005.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%25E9%2585%258D%25E5%259B%25BE.006.png)

放到多台服务器硬盘上，怕一台数据崩了影响数据完整性，那就多复制几份数据在多台服务器备份着。这些存放数据的服务器，就叫 datanode.

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.007.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.008.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.009.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.010.png)

再定义一个 reduce 函数，告诉计算机 map 函数算好的结果怎么汇总起来计算最终结果。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.011.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.012.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.015.png)

它将每个计算任务所需要的资源抽象为**容器**，Container，它本质上其实就是个被限制了 cpu 和内存等计算资源的 jvm 虚拟机进程，容器内运行计算任务代码。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.016.png)

> yarn 的容器跟 docker/k8s 的容器概念上有点像，但终究不是一回事。具体区别是什么，评论区告诉我。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.017.png)

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.018.png)

yarn会根据MapReduce这类计算框架的需求，分配容器资源，再由计算框架将计算任务调度到，已分配的服务器容器上运行。通过一系列的资源申请+协调调度，完成 map 和 reduce 的计算任务并最终得到计算结果。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.019.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.020.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.021.png)

为了让数据分析人员能够像使用 SQL 一样方便地查询大数据，我们需要一个工具来简化这个过程。

它就是 Hive。你可以将它理解为 sql 和 mapreduce 的一个中间层。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.023.png)

它可以将用户输入的类似 SQL 的查询语言解析转换成一个个复杂的 mapreduce 任务，运行并最终输出计算结果。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.024.png)

这样，不会写代码的人也可以通过写 sql 读写大数据。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.022.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%25E9%2585%258D%25E5%259B%25BE.026.png)

以前 Hive 可以将 sql 转化为 Mapreduce 的任务，现在同样也可以通过一个叫 Hive on Spark 的适配层接入 Spark ，将 sql 转化为 spark 任务. 这样 sql 查询的性能也能得到提升。

![](https://cdn.xiaobaidebug.top/hadoop%25E9%2585%258D%25E5%259B%25BE.027.png)

但毕竟 hive 本身不是为 spark 设计的，所以大佬们又设计了 spark sql。它跟 hive 是类似的东西，但跟 spark 搭配起来，性能更好。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.028.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.029.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.030.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.031.png)

## 读数据

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.032.png)
...
![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.033.png)

当然如果追求性能，也可以将 hive 换成 spark sql，将 mapreduce 换成 spark，通过内存加速整个读数据的过程。对于实时性要求高的场景，可以使用flink。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.034.png)

对于在线实时查询场景，可以通过hbase，实现海量数据的毫秒级查询。

![](https://cdn.xiaobaidebug.top/hadoop%E9%85%8D%E5%9B%BE.035.png)
