...
![怎么做到会员优先](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.002.jpeg)

好办，**没有什么是加一层中间层不能解决的，如果有，那就再加一层**。

这次我们要加的中间层是 消息队列 **RabbitMQ**。

![RabbitMQ是什么](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.003.jpeg)
...
![队列是类似链表的独立进程](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.004.jpeg)

它介于**生产者**和**消费者**之间，在流量高峰时先暂存数据，再慢慢消费数据，可以很好的保护消费者，也就是所谓的**削峰填谷**。

![削峰填谷](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.005.jpeg)
...
![A进程挂了不影响其他进程](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.006.jpeg)
...
![Exchange是什么](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.007.jpeg)

这些维护在 Exchange 里的路由方式和绑定关系，我们称为**元数据**。

![元数据是什么](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.008.jpeg)
...
![RabbitMQ是什么](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.009.jpeg)

大佬们在这个架构的基础上，为 RabbitMQ 实现了各种丰富的特性，你能想到的 MQ 功能它基本都实现了，比如**延时队列**，**死信队列**，**优先级队列**等等。

![RabbitMQ的功能](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.010.jpeg)
...
![RabbitMQ集群](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.015.jpeg)

RabbitMQ 支持多种集群模式。我们依次来看下。

![多种集群模式](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.016.jpeg)
...
- 对于**写**操作：生产者将消息写入到 Broker1 的 Queue1 后，Queue1 里的数据并不会同步给其他 broker。但如果此时 Broker1 的 Exchange 元数据有变化，则会将**元数据**同步到其他两个 Broker 中。
  ![写操作](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.018.jpeg)
- 对于**读**操作：消费者想要读取 Queue1 数据时，如果访问的是 Broker1，则直接返回 Queue1 中的数据。
  ![消费消息1](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.019.jpeg)但如果访问的是 Broker2，Broker2 则会根据 Exchange 里的元数据，从 Broker1 那读取数据，再返回给消费者。
  ![消费消息2](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.020.jpeg)
...
![单点问题](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.021.jpeg)
...
![镜像队列集群](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.022.jpeg)

- 对于写操作：数据写入主 Queue 后，会将 Exchange 和 Queue 数据同步给其他 Broker 上。

  ![写操作](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.023.jpeg)
- 对于读操作：消费者读取数据时，如果访问的是主 Queue所在的broker，则直接返回数据。

  ![消费消息1](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.024.jpeg)
- 否则，当前 broker 会从主 queue 所在的 broker 上读取数据，之后返回给消费者。

  ![消费消息2](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.025.jpeg)
...
![基于erlang分布式通信框架同步元数据](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.026.jpeg)

但它有个问题，如果broker间通信断开，镜像队列可能出现多个节点都认为自己是主节点的情况，导致数据不一致，也就是所谓的**脑裂问题**。

![脑裂问题](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.027.jpeg)

有解法吗？有！

我们可以使用更靠谱的一致性算法 **raft** ，来同步多个 broker 的元数据和队列数据，通过引入**选举机制**来解决网络分区问题，这就是所谓的 Quorum 队列集群。

![Quorum队列集群](https://cdn.xiaobaidebug.top/rabbitmq是什么的配图.028.jpeg)
